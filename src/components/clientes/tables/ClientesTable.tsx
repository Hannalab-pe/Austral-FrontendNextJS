  ...
  const columns: ColumnDef<Cliente>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="font-medium">
          ...
          <div className="text-xs text-gray-500">{row.original.numeroDocumento}</div>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          <div className="font-medium">{`${row.original.nombres || ''} ${row.original.apellidos || ''}`.trim()}</div>
          <div className="text-xs text-gray-500">{row.original.emailNotificaciones}</div>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          <div className="text-sm">{row.original.telefono1}</div>
          {row.original.telefono2 && (
            <div className="text-xs text-gray-500">{row.original.telefono2}</div>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const brokerNombre = row.original.asignadoANombre;
        return brokerNombre ? (
